...
public class UserService {
    ...
    public AuthenticationResp createToken(String email) {
        email = email.toLowerCase();
        User user = this.userRepository.findByEmail(email).orElseThrow(
                EmailOrPasswordException::new // Email does not exist
        );
    ...
    public UserPrivateResp createUser(String email, String name, String password, UserType type, String code) {
        if (!verificationCodeRepository.checkVerificationCode(email, code))
            throw new InvalidVerificationCodeException();
        return createUser(email, name, password, type);
    }
    ...
    public String registerVerification(String email) {
        //first check whether the user existed
        if (userRepository.findByEmail(email).isPresent())
            throw new EmailConflictException();

        return verificationCodeRepository.createVerificationCode(email);
    }
